...
(exports.id = 505),
  ...
  (exports.modules = {
    ...
    8694: (l, o, p) => {
      l.exports = p(7096).vendored['react-rsc'].ReactJsxRuntime;
    },
    417: (l, o, p) => {
      ...
      let ImageResponse = class ImageResponse extends Response {
        static #l = (this.displayName = 'NextImageResponse');
        constructor(...l) {
          let o = new ReadableStream({
              async start(o) {
                let a = (await Promise.resolve().then(p.bind(p, 4021)))
                    .ImageResponse,
                  r = new a(...l);
                if (!r.body) return o.close();
                let d = r.body.getReader();
                for (;;) {
                  let { done: l, value: p } = await d.read();
                  if (l) return o.close();
                  o.enqueue(p);
                }
              },
